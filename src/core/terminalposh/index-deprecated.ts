...
const ohMyPosh = 'JanDeDobbeleer.OhMyPosh'
const clink = 'chrisant996.Clink'
const pw = 'Microsoft.PowerShell'
		...
		.action(async () => {
			...
			if (!installPwError) {
				...
				if (fileExistsSync(psProfile)) {
					const config = Array.from(new Set(readFileSync(psProfile).toString().split('\n')))
